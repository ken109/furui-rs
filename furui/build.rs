...
/// This crate has a runtime dependency on artifacts produced by the `furui-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "furui-ebpf")
        .unwrap();
    ...
    // (https://github.com/rust-lang/cargo/issues/4001): Make this `false` if we can determine
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `furui-ebpf`, so cargo will automatically rebuild us
        // if `furui-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `furui-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("furui-ebpf");
