...
unsafe fn try_bind_v4(ctx: ProbeContext) -> Result<u32, u32> {
    ...
    let task = (bpf_get_current_task() as *const task_struct).as_ref().unwrap();

    let nsproxy = bpf_probe_read(&task.nsproxy).unwrap().as_ref().unwrap();
    let pidns = bpf_probe_read(&nsproxy.pid_ns_for_children).unwrap().as_ref().unwrap();
    ...
    let container_id = (&container_id[0..12]).as_ptr().cast::<[c_char; 12]>().read();
